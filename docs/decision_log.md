...
## 2026-08-03 — Phase 4 — adversarial red-team review, one BLOCKING bug found and fixed, novelty framing corrected

**Decision:** Ran an independent Opus red-team subagent against the full Phase 3 output (derivation.md, all source code, all tests, all figures) per the program's Phase 4 mandate. It found and this program fixed a BLOCKING implementation bug, two SERIOUS convention/enforcement errors, and — most consequential — found the novelty framing anchored to the wrong field. All findings and fixes are logged exhaustively in `redteam_report.md` (the red-team's report, left unedited as the primary record) and `limitations.md` §2 (the current, corrected account). Summarized here for the audit trail:

1. **`dormancy.py` omitted the eq. (5.3) biofilm growth term.** This had produced a spurious ~2.2×10⁵× lunar/Earth dormancy-deposit ratio, presented as the derivation's "strongest single result." Restoring the term (exactly as the derivation specifies) collapses the ratio to ~0.96 — the long-time outcome is nearly gravity-independent because growth kinetics, not transport, set the final substrate-limited ceiling once any deposit seeds. The real, robust, transport-driven effect turned out to be the deposition *rate* (~1700x difference between Earth/Mars and Moon, via the Rayleigh-number convection on/off switch), not the final accumulated mass. Figure and reporting rebuilt around the corrected finding rather than silently kept.
2. **Two SERIOUS convention/enforcement errors, both fixed:** `Λ_g` used a channel half-depth while claiming to be the classical (full-depth) Hazen number, flipping one size-threshold claim; and the Reynolds-number validity bound on Stokes' law was enforced in `particles.py` but not in the closed-form criteria functions that most figures actually call, silently biasing results for large flocs. Both fixed with regression tests added.
3. **The wall-orientation convention (`cos θ=1` vs. the perimeter-averaged `f_θ=1/π`) was inconsistent between §3.2 and §5.3 of the derivation, and the headline `g*` used the less-defensible one.** Fixed by making orientation an explicit parameter, defaulting to the physically-derived perimeter average, and testing both conventions rather than silently picking one.
4. **Novelty framing was anchored to the wrong field.** The Phase 2 adjudication's own diagnosed failure mode — a search scoped to the obviously-relevant literature missing disqualifying/contextualizing prior art in an adjacent field — recurred here: colloid-filtration theory (Yao/Habibian/O'Melia 1971, Tufenkji/Elimelech 2004, Pich 1972, Belfort/Davis/Zydney 1994) already owns the diffusion+interception+sedimentation decomposition this program built, and an ICES paper (SAE 2009-01-2359) already did partial-gravity particle transport in water hardware. `problem_statement.md` §7 restates the contribution honestly: adding a gravity axis to an established 50-year-old regime decomposition, evaluated at ECLSS-specific operating points — not deriving a new criterion. This is logged as a second instance of the same methodology lesson from Phase 2 (see the 2026-07-31 Phase 2 entry above), now generalized in `limitations.md` §3 for future phases.

**What was disclosed rather than fixed** (deliberate, not an oversight): the `Ra_c=1708` convection-threshold requires a vertical destabilizing thermal gradient a real spacecraft line will not generically have (upgraded from a minor caveat to a prominent one, not code-changed, since the formula itself is correct for the case it models); the general spatial 1-D PDE (derivation §5.1-5.2, §9.7) was never implemented, only the closed-form criteria and the 0-D stagnant reduction — consistent with the derivation's own statement that the PDE isn't needed to locate the regime boundary; the steady-flow assumption (WPA duty-cycling) and negative-buoyant-density (`Δρ<0`, rising aggregates) cases remain unexplored in the Monte Carlo and figures.

**Confidence:** the criterion framework (§§2-3 of the derivation) — HIGH, independently re-derived to ≤0.3% by an adversarial reviewer with no stake in it being right. The stagnant/dormancy branch as a headline result — LOW before this fix, MEDIUM after (rate-based claim, not the retracted mass-based one). The novelty claim as reframed in `problem_statement.md` §7 — MEDIUM (the red-team's own assessment, adopted).

**Rationale for logging this so exhaustively:** this is exactly what Phase 4 is for, and exactly what the program's integrity rules ask for — a paper that names its own weaknesses is stronger than one that hides them. The corrections substantially narrow and modest-ify the paper's claims relative to the Phase 3 output, and that is treated here as the system working as intended, not as a setback to minimize.
